...
//!
//! **No behavior.** The `greet` implementation lives in `crates/tinyloops`,
...
//!
//! `tinyloops` **depends on this crate and re-exports all of it**, so
//! `tinyloops::GreetRequest` and `tinyloops_bus::greeting::GreetRequest` are the
...
//!
//! So: a module author depends on `tinyloops` and gets behavior and vocabulary.
//! A host depends on `tinyloops-bus` and gets vocabulary alone.
//!
//! # Staying in step with the module
//!
//! [`names::METHODS`] lists every member. `crates/tinyloops` asserts its served
...
//! assert_eq!(names::OBJECT_PATH, "/ai/tinyhumans/tinyloops/Greeting");
//!
