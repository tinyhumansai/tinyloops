...
1. Failing tests, all against one base state and hand-built arm outputs:
   ...
   - `a_list_folds_by_what_each_arm_appended`, and
     ...
     existing `Error::ContestedField { field, held_by, also }`. Add no second
     variant: `state` landed the arbitration in
     `LoopState::merge(&[Delta], &[Contribution])`, and a parallel error type
     would be a second place for one rule to be wrong.
...
2. Implement:
   ...
   pub trait ArmFold: Send + Sync {
       ...
       // Delegates to `LoopState::merge`: a `Delta` for the counters, a
       // `Contribution` for the fields the arm owns.
